...
import { UnstructuredStorageInstance } from '../../types/truffle-contracts';
import { UnstructuredStorage } from '../artifacts';
import { configure } from '../configure';

describe('UnstructuredStorage', function () {
    let web3: Web3
    let accounts: string[]
    let storage: UnstructuredStorageInstance
    ...
    beforeEach(async () => {
        storage = await UnstructuredStorage.new()
    });

    it('getUInt256(), setUInt256()', async () => {
        const slot = Web3.utils.keccak256('UnstructuredStorage.uint256')
    ...
    it('getAddress(), setAddress()', async () => {
        const slot = Web3.utils.keccak256('UnstructuredStorage.address')
    ...
    it('getUInt256Array(), pushUInt256Array()', async () => {
        const slot = Web3.utils.keccak256('UnstructuredStorage.mapping(uint256,uint256)')
        const key = 42
