...
/**
 * @dev An optimized uint16 min/max heap
 *
...
library HeapLibUInt16 {
     ...
     * @return value
     */
    function root(bytes32 slot) internal view returns (uint16 value) {
        value = slot.get(0);
    }
    ...
    function leaves(
        ...
        uint16[] memory le = new uint16[](lIdx.length);
    ...
    function set(
        ...
        if (compare(slot.get(i), slot.get(TreeLib.parentIdx(a, b, i)), compareSelector)) {
            // heap[i] < parent (min-heap), heapifyUp
            heapifyUp(slot, a, b, compareSelector, i);
        } else {
            // heap[i] > parent (min-heap), heapifyDown
    ...
    function searchUp(
        ...
        uint256 length = slot.len();
        require(idx < length, 'HeapLibUInt16.searchUp: idx out of bounds.');
    ...
    function searchDown(
        ...
        revert('HeapLibUInt16.searchDown: v not found.'); //No more leaves to search. These get removed recursively as they are either out of bounds or cannot be parents to the search value.
    }
