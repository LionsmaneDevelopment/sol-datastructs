...
/**
 * @dev An optimized uint32 min/max heap
 *
...
library HeapLibUInt32 {
     ...
     */
    function get(bytes32 slot, uint256 i) internal view returns (uint32 value) {
        value = slot.get(i);
    }
     ...
     * @return value
     */
    function root(bytes32 slot) internal view returns (uint32 value) {
        value = slot.get(0);
    }
    ...
    function leaves(
        ...
        uint32[] memory le = new uint32[](lIdx.length);
    ...
    function searchUp(
        ...
        uint256 length = slot.len();
        require(idx < length, 'HeapLibUInt32.searchUp: idx out of bounds.');
    ...
    function searchDown(
        ...
        revert('HeapLibUInt32.searchDown: v not found.'); //No more leaves to search. These get removed recursively as they are either out of bounds or cannot be parents to the search value.
    }
