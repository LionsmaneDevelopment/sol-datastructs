...
/**
 * @dev A gas optimized uint32[] implementation. Uses the ArrayLibUInt with bitLength = 32.
 *
 *
 */
library ArrayLibUInt32 {
    /**
    ...
    function get(bytes32 slot, uint256 i) internal view returns (uint32 value) {
        value = uint32(ArrayLibUInt.get(32, slot, i));
    }
    ...
    function set(
        ...
        ArrayLibUInt.set(32, slot, i, val);
    }
     ...
     * @param val value
     */
    function push(bytes32 slot, uint32 val) internal {
        ArrayLibUInt.push(32, slot, val);
    }
    ...
    function getBatch(bytes32 slot, uint256[] memory iArray) internal view returns (uint256[] memory valList) {
        valList = ArrayLibUInt.getBatch(32, slot, iArray);
    }
    ...
    function setBatch(
        ...
        ArrayLibUInt.setBatch(32, slot, iArray, valArray);
    }
    ...
    function pushBatch(bytes32 slot, uint256[] memory valArray) internal {
        ArrayLibUInt.pushBatch(32, slot, valArray);
    }
    ...
    function popBatch(bytes32 slot, uint256 n) internal {
        ArrayLibUInt.popBatch(32, slot, n);
    }
}
