...
/**
 * @dev A gas optimized uint128[] implementation. Uses the ArrayLibUInt with bitLength = 128.
 *
 *
 */
library ArrayLibUInt128 {
    /**
     ...
     *
     */
    function get(bytes32 slot, uint256 i) internal view returns (uint128 value) {
        value = uint128(ArrayLibUInt.get(128, slot, i));
    }
    ...
    function set(
        ...
        ArrayLibUInt.set(128, slot, i, val);
    }
     ...
     * @param val value
     */
    function push(bytes32 slot, uint128 val) internal {
        ArrayLibUInt.push(128, slot, val);
    }
    ...
    function swap(
        ...
        ArrayLibUInt.swap(128, slot, i, j);
    }
    ...
    function getBatch(bytes32 slot, uint256[] memory iArray) internal view returns (uint256[] memory valList) {
        valList = ArrayLibUInt.getBatch(128, slot, iArray);
    }
    ...
    function setBatch(
        ...
        ArrayLibUInt.setBatch(128, slot, iArray, valArray);
    }
    ...
    function pushBatch(bytes32 slot, uint256[] memory valArray) internal {
        ArrayLibUInt.pushBatch(128, slot, valArray);
    }
    ...
    function popBatch(bytes32 slot, uint256 n) internal {
        ArrayLibUInt.popBatch(128, slot, n);
    }
}
